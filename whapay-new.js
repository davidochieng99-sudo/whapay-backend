...
// whapay-new.js - Backend routes only (no server startup)
require('dotenv').config();
const express = require('express');
const cors = require('cors');
const admin = require('firebase-admin');
...
admin.initializeApp({
  ...
  databaseURL: 'https://whapay-c93ab.firebaseio.com'
});
const db = admin.firestore();

// Flutterwave
const FLW_PUBLIC_KEY = process.env.FLW_PUBLIC_KEY || 'FLWPUBK_TEST-xxxxxxxxxxxxxxxxxxxxx';
const FLW_SECRET_KEY = process.env.FLW_SECRET_KEY || 'FLWSECK_TEST-xxxxxxxxxxxxxxxxxxxxx';
...
function formatPhone(phone) {
  ...
  if (cleaned.length !== 12) throw new Error('Invalid phone number format');
  return cleaned;
}
...
app.post('/api/mpesa/charge', async (req, res) => {
  try {
    ...
    const tx_ref = generateTxRef('MPESA');
    ...
    const payload = {
      ...
      tx_ref,
      payment_type: 'mpesa',
      redirect_url: process.env.REDIRECT_URL || 'https://your-domain.com/payment-callback'
    };
    ...
    if (response.status === 'success') {
      await db.collection('transactions').add({
        ...
        status: 'pending',
        tx_ref,
        flutterwaveId: response.data?.id || null,
...
// ==================== 2. MOBILE MONEY (AIRTEL, MTN, TIGO, ORANGE, VODAFONE) ====================
app.post('/api/mobile-money/charge', async (req, res) => {
  try {
    ...
    const phone = formatPhone(customerPhone);
    const tx_ref = generateTxRef('MOBILE');
    ...
    switch (network.toUpperCase()) {
      case 'AIRTEL':
        payload.payment_type = 'airtel';
        response = await flw.MobileMoney.airtel(payload);
        break;
      case 'MTN':
        payload.payment_type = 'mtn';
        response = await flw.MobileMoney.mtn(payload);
        break;
      case 'TIGO':
        payload.payment_type = 'tigo';
        response = await flw.MobileMoney.tigo(payload);
        break;
      case 'ORANGE':
        payload.payment_type = 'orange';
        response = await flw.MobileMoney.orange(payload);
        break;
      case 'VODAFONE':
        payload.payment_type = 'vodafone';
        response = await flw.MobileMoney.vodafone(payload);
        break;
      default:
    ...
    if (response.status === 'success') {
      ...
      throw new Error(response.message || `${network} payment failed`);
    }
  } catch (error) {
    console.error('Mobile money error:', error.message);
    res.status(500).json({ success: false, error: error.message });
  }
});
...
app.post('/api/card/charge', async (req, res) => {
  try {
    ...
    const tx_ref = generateTxRef('CARD');
    const payload = {
...
app.post('/api/register-pay', async (req, res) => {
  try {
    ...
    let memberCode = null;
    if (registerUser) {
      const prefix = fullname.replace(/[^a-zA-Z0-9]/g, '').substring(0, 3).toUpperCase() || 'NEW';
      let code, exists = true;
      while (exists) {
        const random = Math.floor(10000 + Math.random() * 90000);
        code = `${prefix}${random}`;
        const snapshot = await db.collection('users').where('memberCode', '==', code).get();
...
app.post('/api/verify-transaction', async (req, res) => {
  try {
    ...
    if (!tx_ref) return res.status(400).json({ success: false, error: 'Missing tx_ref' });
    const response = await flw.Transaction.verify({ id: tx_ref });
    if (response.status === 'success') {
      const transactions = await db.collection('transactions').where('tx_ref', '==', tx_ref).get();
      transactions.forEach(doc => {
        doc.ref.update({ status: response.data.status, flutterwaveRef: response.data.flw_ref });
      });
...
app.post('/api/flutterwave-webhook', express.json({ type: 'application/json' }), async (req, res) => {
  const event = req.body;
  if (event.event === 'charge.completed') {
    const { tx_ref, status, flw_ref } = event.data;
    const transactions = await db.collection('transactions').where('tx_ref', '==', tx_ref).get();
    transactions.forEach(doc => doc.ref.update({ status, flutterwaveRef: flw_ref }));
  }
  res.status(200).json({ status: 'success' });
});
