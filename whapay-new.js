...
// ========================
const WHATSAPP_API_VERSION = 'v20.0';  // use latest version
...
async function sendWhatsAppMessage(phoneNumber, message, messageType = 'text') {
  ...
  try {
    ...
    console.error(`❌ WhatsApp failed to ${phoneNumber}:`, error.response?.data || error.message);
    return { success: false, error: error.message };
  }
}

// Send utility receipt (order confirmation, payment receipt)
async function sendWhatsAppReceipt(phoneNumber, receiptData) {
  const { transactionId, amount, merchantName, description } = receiptData;
  const message = `*WhaPay Receipt*\n━━━━━━━━━━━━━━━━━━━━━\nTransaction: ${transactionId}\nAmount: KES ${amount}\nMerchant: ${merchantName}\nDescription: ${description || "Payment"}\n━━━━━━━━━━━━━━━━━━━━━\nThank you for using WhaPay!`;
  return await sendWhatsAppMessage(phoneNumber, message);
}

// Webhook to receive incoming WhatsApp messages (optional)
app.post('/webhooks/whatsapp', (req, res) => {
  ...
  if (body.entry && body.entry[0]?.changes?.[0]?.value?.messages) {
    ...
    console.log(`📨 Incoming WhatsApp from ${customerPhone}: ${messageText}`);
    // Process reply here if needed
  }
