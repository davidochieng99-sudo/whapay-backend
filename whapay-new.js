...
async function registerOrGetUser(phoneNumber, fullname = null, userType = "customer") {
  ...
  const existingUser = await usersRef.where("phoneNumber", "==", normalizedPhone).get();
  
  if (!existingUser.empty) {
    let userDoc = existingUser.docs[0];
    return {
      id: userDoc.id,
      dkCode: userDoc.data().dkCode,
      qrCodeUrl: userDoc.data().qrCodeUrl,
...
h1 { color: #25D366; }
.btn { display: inline-block; padding: 15px 30px; margin: 10px; background: #25D366; color: white; text-decoration: none; border-radius: 5px; }
</style>
</head>
<body>
<div class="container">
...
function readQRCode(input) {
  alert('QR code detected. Merchant code extracted.');
}
...
async function sendLink() {
  ...
  const response = await fetch('/api/create-link', {
    ...
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      merchantCode: currentUser.dkCode,
      customerPhone: phone,
      amount: parseFloat(amount),
      description: desc
    })
...
app.post("/api/create-link", async (req, res) => {
  try {
    ...
    const paymentLink = `http://localhost:3001/pay?code=${merchantCode}`;
    const message = `Pay KES ${amount} to ${merchantData.fullname} (${merchantData.dkCode}) for ${description || "payment"}. Click: ${paymentLink}`;
    const whatsappLink = `https://wa.me/${normalizedPhone}?text=${encodeURIComponent(message)}`;
    
    res.json({ success: true, whatsapp_link: whatsappLink });
  } catch (error) {
    res.json({ success: false, error: error.message });
  }
});
// Dexatel SMS endpoint
app.post("/api/send-sms", async (req, res) => {
  try {
    const { to, message } = req.body;
    
    // Check if Dexatel API key exists
    if (!process.env.DEXATEL_API_KEY) {
      throw new Error("Dexatel API key not configured");
    }
...
app.listen(PORT, () => {
  ...
  console.log(`Open http://localhost:3001 to test`);
});